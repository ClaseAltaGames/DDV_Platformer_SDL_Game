...
WEnemies3::WEnemies3() : Entity(EntityType::WENEMIES)
{
	name.Create("wenemy");
...
void WEnemies3::WEnemiesStartAnims()
{
...
bool WEnemies3::Update(float dt)
{
	dt = 16;
...
void WEnemies3::OnCollision(PhysBody* physA, PhysBody* physB)
{
	switch (physB->ctype)
	{
...
void WEnemies3::MoveToDestination(float dt)
{
	// Update the character's position
	b2Transform ebodyPos = ebody->body->GetTransform();
	position.x = METERS_TO_PIXELS(ebodyPos.p.x) - 16 / 2;
...
void WEnemies3::MoveToOrigin(float dt)
{
...
bool WEnemies3::HasReachedDestination()
{
	// Update the character's position
	b2Transform ebodyPos = ebody->body->GetTransform();
	position.x = METERS_TO_PIXELS(ebodyPos.p.x) - 16 / 2;
...
bool WEnemies3::HasReachedOrigin()
{
	// Update the character's position
	b2Transform ebodyPos = ebody->body->GetTransform();
	position.x = METERS_TO_PIXELS(ebodyPos.p.x) - 16 / 2;
...
Animation* WEnemies3::GetAnimation(SString name)
{
